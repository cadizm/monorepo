 ...
 * many items by the time its turn comes around. If a monkey is holding no items at the start of its
 * turn, its turn ends.
 *
...
public class Day11MonkeyInTheMiddle {
  ...
  public static long monkeyBusinessLevel() {
    Stream<String> lines = Resource.readLines("day11.input");

    SortedMap<Integer, Monkey> monkeyMap = parseInput(lines);

    for (int i = 0; i < 20; ++i) {
      monkeyRound(monkeyMap, false);  // use "original" relief
    }
  ...
  public static long monkeyBusinessLevelAlternateRelief() {
    Stream<String> lines = Resource.readLines("day11.input");

    SortedMap<Integer, Monkey> monkeyMap = parseInput(lines);

    for (int i = 0; i < 10_000; ++i) {
      monkeyRound(monkeyMap, true);  // use alternative relief
    }
  ...
  static void monkeyRound(Map<Integer, Monkey> monkeyMap, boolean alternate) {
    for (var entry : monkeyMap.entrySet()) {
      Monkey monkey = entry.getValue();
      monkeyTurn(monkey, monkeyMap, alternate);
    }
  }

  static void monkeyTurn(Monkey monkey, Map<Integer, Monkey> monkeyMap, boolean alternate) {
    while (!monkey.items.isEmpty()) {
      long item = monkey.items.pollFirst();
      monkey.count += 1;
      String expr = monkey.operation.replaceAll("old", String.format("%s", item));
      if (!alternate) {
        // original way to avoid 64-bit overflow is to use
        // arbitrary number "3" to divide item worry level
        item = evaluate(expr) / 3;
      } else {
        // An alternative method is to use "modular arithmetic" with a
        // common base across all the monkey test divisors (so as to
        // consistently compute which monkey to throw the item to). We
        // use the lcm of all the divisors so that the divisibility
        // test for any monkey divisor remains consistent.
        item = evaluate(expr) % lcm(monkeyMap);
      }
      boolean flag = item % monkey.testDivisor == 0;
      int dst = flag ? monkey.trueDst : monkey.falseDst;
      monkeyMap.get(dst).items.offerLast(item);
    }
  }
  ...
  static long evaluate(String expression) {
    ...
    if (parts[1].equals("*")) {
      return Long.parseLong(parts[0]) * Long.parseLong(parts[2]);
    } else if (parts[1].equals("+")) {
      return Long.parseLong(parts[0]) + Long.parseLong(parts[2]);
    }

    throw new RuntimeException(String.format("Don't know how to handle operator %s", parts[1]));
  }

  static SortedMap<Integer, Monkey> parseInput(Stream<String> input) {
    SortedMap<Integer, Monkey> monkeyMap = new TreeMap<>();
    ...
    for (var it = input.iterator(); it.hasNext(); ) {
      ...
      if (s.startsWith("Monkey")) {
        int num = Integer.parseInt(s.replaceAll(":", "").split(" ")[1]);
        monkey.num = num;
      } else if (s.startsWith("Starting items")) {
        List<Long> items = Stream.of(s.split(": ")[1].split(", "))
            .map(Long::valueOf)
            .collect(Collectors.toList());
        monkey.items = new ArrayDeque<>(items);
      } else if (s.startsWith("Operation")) {
        monkey.operation = s.split("new = ")[1];
      } else if (s.startsWith("Test")) {
        monkey.testDivisor = Integer.parseInt(s.split("divisible by ")[1]);
      } else if (s.startsWith("If true")) {
        monkey.trueDst = Integer.parseInt(s.split("throw to monkey ")[1]);
      } else if (s.startsWith("If false")) {
        monkey.falseDst = Integer.parseInt(s.split("throw to monkey ")[1]);
      } else if (s.isBlank()) {
        monkeyMap.put(monkey.num, monkey);
        monkey = new Monkey();
      } else {
        throw new RuntimeException(String.format("Don't know how to handle input %s", s));
      }
    }
  ...
  /*
   * Because all of the testDivisors are primes, their LCM is equal to their product
   * TODO: update with real lcm algorithm and compare
   */
  static int lcm(Map<Integer, Monkey> monkeyMap) {
    return monkeyMap.values()
        .stream()
  ...
  static class Monkey {
    ...
    public String toString() {
      return String.format(
          "Monkey {num=%s count=%s items=%s operation=%s test=%s true=%s false=%s}",
          num, count, items, operation, testDivisor, trueDst, falseDst);
    }
