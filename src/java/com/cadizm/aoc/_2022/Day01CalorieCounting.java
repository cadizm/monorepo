...
public class Day01CalorieCounting {
   ...
   */
  public static long maxElfCalories() {
   ...
   * in total?
   */
  public static long sumTop3ElfCalories() {
    Stream<String> lines = Resource.readLines("day01.input");

    PriorityQueue<Long> sums = new PriorityQueue<>(Comparator.reverseOrder());  // max heap

    String s = "0";
    long sum = 0;

    for (var it = lines.iterator(); it.hasNext(); s = it.next()) {
      if (s.isBlank()) {
        sums.add(sum);
        sum = 0;
      } else {
    ...
    sum += Long.parseLong(s);  // last line
    sums.add(sum);  // last "grouping"

    sum = 0;
    for (int i = 0; i < 3 && !sums.isEmpty(); ++i) {
      sum += sums.poll();
    }
