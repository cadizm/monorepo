...
public class Day03RucksackReorganization {
   ...
   */
  public static int prioritySum() {
    Stream<String> lines = Resource.readLines("day03.input");
    ...
    for (var it = lines.iterator(); it.hasNext(); ) {
      ...
      for (int i = 0; i < n / 2; ++i) {
        r1.add(s.charAt(i));
        r2.add(s.charAt(i + (n / 2)));
      }

      r1.retainAll(r2);  // set intersection
      assert r1.size() == 1;

      sum += score(r1.iterator().next());
    }
   ...
   */
  public static int prioritySum3Elves() {
    Stream<String> lines = Resource.readLines("day03.input");

    int sum = 0;

    for (var it = lines.iterator(); it.hasNext(); ) {
      Set<Character> a = toSet(it.next());
      Set<Character> b = toSet(it.next());
      Set<Character> c = toSet(it.next());

      a.retainAll(b);
      a.retainAll(c);
      assert a.size() == 1;

      sum += score(a.iterator().next());
    }
