...
public class Day10CathodeRayTube {
  ...
  public static int signalStrength() {
    Stream<String> lines = Resource.readLines("day10.input");

    int x = 1;
    int cycle = 1;

    TreeMap<Integer, Integer> map = new TreeMap<>();  // map from cycle -> x value at cycle
    ...
    for (var it = lines.iterator(); it.hasNext(); ) {
      ...
      if (s.equals("noop")) {
        ...
        throw new RuntimeException(String.format("Unknown instruction: %s", s));
      }
    }

    int sum = 0;
    for (int i = 20; i <= 220; i += 40) {
      // get entry with greatest key <= to given key
      Map.Entry<Integer, Integer> entry = map.floorEntry(i);
   ...
   */
  public static String racingTheBeam() {
    Stream<String> lines = Resource.readLines("day10.input");
  ...
  static String render(TreeMap<Integer, Integer> map) {
    ...
    for (int i = 0; i < rows; ++i) {
      for (int j = 0; j < cols; ++j) {
        int cycle = (cols * i) + j + 1;
        Map.Entry<Integer, Integer> entry = map.floorEntry(cycle);  // get register value at cycle

        int pixel = j;  // current pixel to render
        int sprite = entry.getValue(); // middle value of sprite's 3 pixels

        if (Math.abs(sprite - pixel) <= 1) {  // if sprite is visible
          sb.append("#");  // set pixel on
        } else {
          sb.append(".");  // set pixel off
        }

        if ((j + 1) % 5 == 0) {
          sb.append(" ");  // add space at pixel letter boundaries
        }
      }
      sb.append("\n");  // advance to next row to render
    }
